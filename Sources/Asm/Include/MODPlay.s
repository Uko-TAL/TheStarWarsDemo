...
; 2020/12/24: Make sure that one one the three steps (mp_read_song, mp_premix, mp_mix) is not skipped
...
; -----------------------------------------------------------------------------
mp_read_song:	; Since the process at each VBL is split in 3 steps (mp_read_song, mp_premix, mp_mix), and that each of
	; these steps may be performed in a different interrupt (VBL, timerA, ...), we must ensure that all steps
	; have been performed before starting a new sequence. This could for example occur when changing of screens
	tst.w mp_mix_step
	bne .exit
...
MP_NOPS_MIX_STD_VOL_END	equ 23	
...
MP_NOPS_MIX_OVS_VOL_END	equ 22
...
MP_NOPS_MIX_EXT_VOL_END	equ 22
...
mp_mix_step	ds.w 1	; Current step to be performed: 0 = mp_read_song, 1 = mp_premix, 2 = mp_mix
