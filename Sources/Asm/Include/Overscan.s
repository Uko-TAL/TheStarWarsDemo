...
; Screen size
; 408x273 but
; - line length is 224 bytes
; - we will use only 400 pixels (multiple of 16)
	ifnd FS_SCRLINE_LEN
FS_SCRLINE_LEN	equ 224
FS_DISPLINE_LEN	equ 200
	endif
	
FS_SCR_NBLINES	equ 273

; -----------------------------------------------------------------------------
; Line Sequence
; Open Left Border (24 cycles)	: ends at pixel -32
; 6 NOPS (24 cycles)	; ends at pixel - 8 (left border)
; 2 NOPs (16 cycles)	; ends at pixel 0 (begin of displayable line) 
; 82 NOPs (328 cycles)	: ends at pixel 328
; Open Right Border (24 cycles)	: ends at pixel 352
; 14 NOPs (56 cycles)	: ends at pixel 408 (end of displayable line)
; 12 NOPS (48 cycles)	; ends at pixel 456
; Open Left Border (24 cycles)	: ends at pixel 488
; 6 NOPS (24 cycles)	; ends at pixel 512 (left border)
; ...
; -----------------------------------------------------------------------------
...
overscan_callTimA: macro
	...
	; 4 = 1/50 = 49152 Hz, roughly 1000 * VBL frequency, so roughly every 0.3 lines (313 lines in a vbl)
...
; Launch the Timer B to remove the low border 
; To be called at the very beginning of the VBL just after overscan_callTimA !
; \1 = timer B routine
overscan_callTimB: macro
	timer_enable "B",\1
	timer_start "B",HW_TIMB_HBL_CTL,OVERSCAN_BOTTOM_BORDER_NBSCANLINE
	endm


; Timer A routine that removes the top border
; WARNING: d7 can no longer be used for demo (only for overscan)
; \1 = Number of NOPS required outside this macro
...
.sync\@:	move.b (a0),d2 ;8 
	...
	ifne \#
	nops OVERSCAN_NOPS_END_TOP_BORDER-OVERSCAN_NOPS_BEFORE_LEFT_BORDER-9-(\1)
	else
...
overscan_lowborder: macro
	timer_stop "B"
	; Save registers, because if we remove only the low border, we may have things
	; running in background !
	movem.l d0-d1,-(sp)
	; Be sure to be outside border
	;move.w #$0700,HW_ST_PAL_ADR.w
	nops 50
	;move.w #$0070,HW_ST_PAL_ADR.w
	; Then wait to be in the right border
.inScreen	move.b (HW_SHIFTER_ADR+$9).w,d0
	move.b (HW_SHIFTER_ADR+$9).w,d1
	cmp.b d0,d1
	bne.s .inScreen
	; Wait half of the border
	;move.w #$0000,HW_ST_PAL_ADR.w
	nops 8
	; 60/50 Hz switch
	move.b #0,(HW_SHIFTER_ADR+$A).w
	nops 9
